#ifndef		__GAIA_JSON_JSON_IMPL_H__
#define		__GAIA_JSON_JSON_IMPL_H__
...
namespace GAIA
{
	namespace JSON
	{
		GINL Json::Json()
		{
			m_root.m_pszName = GAIA::JSON::JSON_DEFAULT_ROOT_NODE_NAME;
			m_root.SetRoot(GAIA::True);
			m_root.SetType(GAIA::JSON::JSON_NODE_CONTAINER);
		}
		GINL Json::~Json()
		{
			this->Reset();
		}
		GINL GAIA::GVOID Json::Reset()
		{
			if(m_root.m_pszName == GAIA::JSON::JSON_DEFAULT_ROOT_NODE_NAME)
						...
						case GAIA::JSON::JSON_NODE_CONTAINER:
							{
								...
								else
								{
									if(cur.pCurrentNode != GNIL)
										cur.pCurrentNode->SetType(GAIA::JSON::JSON_NODE_CONTAINER);
						...
						case GAIA::JSON::JSON_NODE_MULTICONTAINER:
							{
								...
								{
									if(cur.pCurrentNode != GNIL)
										cur.pCurrentNode->SetType(GAIA::JSON::JSON_NODE_MULTICONTAINER);
									this->EndWriteNode(cur);
								}
							}
							break;
		...
		GINL GAIA::BL Json::SaveToFile(const GAIA::TCH* pszFileName, GAIA::JSON::JSON_SAVE st)
		{
			...
			GAST(st == GAIA::JSON::JSON_SAVE_BESTSIZE ||
				 st == GAIA::JSON::JSON_SAVE_BESTREAD);
			if(st != GAIA::JSON::JSON_SAVE_BESTSIZE &&
					st != GAIA::JSON::JSON_SAVE_BESTREAD)
				return GAIA::False;
		...
		GINL GAIA::BL Json::LoadFromMem(const GAIA::TCH* pszSrc, const GAIA::NUM& sSize)
		{
			GAST(pszSrc != GNIL);
			if(pszSrc == GNIL)
				return GAIA::False;
		...
		GINL GAIA::BL Json::SaveToMem(GAIA::TCH* pszDst, const GAIA::NUM& sSize, GAIA::JSON::JSON_SAVE st)
		{
			GAST(pszDst != GNIL);
			if(pszDst == GNIL)
				return GAIA::False;
		...
		GINL JsonNode& Json::GetRootNode()
		{
			return m_root;
		}
		GINL const JsonNode& Json::GetRootNode() const
		{
			return m_root;
		}
		GINL GAIA::BL Json::BeginWriteNode(GAIA::JSON::Json::Cursor& cur, GAIA::JSON::JSON_NODE nt, const GAIA::TCH* pszNodeName)
		{
			if(cur.pCurrentNode == GNIL)
			{
			...
			else
			{
				...
				GAIA::JSON::JsonNode* pNewNode = (pFactory != GNIL) ? (&pFactory->CreateNode()) : (gnew GAIA::JSON::JsonNode);
				pNewNode->SetType(nt);
		...
		GINL GAIA::BL Json::EndWriteNode(GAIA::JSON::Json::Cursor& cur)
		{
			if(cur.pCurrentNode == GNIL)
				return GAIA::False;
		...
		GINL GAIA::BL Json::WriteNode(GAIA::JSON::Json::Cursor& cur, GAIA::JSON::JSON_NODE nt, const GAIA::TCH* pszNodeName)
		{
		...
		GINL const GAIA::TCH* Json::BeginReadNode(GAIA::JSON::Json::Cursor& cur) const
		{
			return GNIL;
		}
		GINL GAIA::BL Json::EndReadNode(GAIA::JSON::Json::Cursor& cur) const
		{
			return GAIA::True;
		}
		GINL const GAIA::TCH* Json::ReadNode(GAIA::JSON::Json::Cursor& cur) const
		{
			return GNIL;
		}
		GINL GAIA::GVOID Json::write_linebreak(GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f)
		{
			if(st != GAIA::JSON::JSON_SAVE_BESTREAD)
				return;
			f.WriteText(GAIA_FILELINEBREAK);
		}
		GINL GAIA::GVOID Json::write_indent(GAIA::JSON::JsonNode& n, GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f)
		{
			if(st != GAIA::JSON::JSON_SAVE_BESTREAD)
				return;
		...
		GINL GAIA::GVOID Json::write_string(GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f, const GAIA::CH* p, GAIA::NUM sLen)
		{
		...
		GINL GAIA::GVOID Json::write_string(GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f, const GAIA::WCH* p, GAIA::NUM sLen)
		{
			GAST(!GAIA::ALGO::gstremp(p));
			if(GAIA::ALGO::gstremp(p))
				return;
		...
		GINL GAIA::GVOID Json::save_node(GAIA::JSON::JsonNode& n, GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f)
		{
			switch(n.GetType())
			{
			case GAIA::JSON::JSON_NODE_CONTAINER:
		...
		GINL GAIA::GVOID Json::save_child_node(GAIA::JSON::JsonNode& n, GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f)
		{
			for(GAIA::NUM x = 0; x < n.GetChildCount(); ++x)
			{
				GAIA::JSON::JsonNode* pChild = n.GetChild(x);
				if(pChild == GNIL)
					continue;
				GAIA::JSON::JSON_NODE childtype = pChild->GetType();
				if(childtype != GAIA::JSON::JSON_NODE_CONTAINER &&
						childtype != GAIA::JSON::JSON_NODE_MULTICONTAINER)
					continue;
				this->save_node(*pChild, st, f);
			}
		}
		GINL GAIA::GVOID Json::save_child_node_value(GAIA::JSON::JsonNode& n, GAIA::JSON::JSON_SAVE st, GAIA::FSYS::FileBase& f)
		{
			for(GAIA::NUM x = 0; x < n.GetChildCount(); ++x)
			{
				GAIA::JSON::JsonNode* pChild = n.GetChild(x);
				if(pChild == GNIL)
					continue;
				GAIA::JSON::JSON_NODE childtype = pChild->GetType();
				if(childtype != GAIA::JSON::JSON_NODE_NAME &&
						childtype != GAIA::JSON::JSON_NODE_VALUE)
					continue;
				if(childtype == GAIA::JSON::JSON_NODE_NAME)
					this->write_string(st, f, " ", 1);
				this->save_node(*pChild, st, f);
		...
		GINL GAIA::NUM Json::load_node(GAIA::U8* pBuf, GAIA::NUM sSize, GAIA::JSON::JSON_NODE& nt, GAIA::U8*& pBegin, GAIA::U8*& pEnd, GAIA::BL& bBegin) const
		{
			GAST(pBuf != GNIL);
			if(pBuf == GNIL)
				return GINVALID;
			...
			for(GAIA::NUM x = 0; x < sSize; ++x)
			{
				if(pBuf[x] == '<')
				{
					if(x + 1 >= sSize)
						return GINVALID;
					if(pBuf[x + 1] == '/') // Multi-Container node end.
					{
						if(x + 3 >= sSize)
							return GINVALID;
						GAIA::NUM sRet;
		...
		GINL GAIA::NUM Json::load_head(GAIA::U8* pBuf, GAIA::NUM sSize, GAIA::NUM sOffset, GAIA::NUM& sNext) const
		{
		...
		GINL GAIA::NUM Json::load_comment(GAIA::U8* pBuf, GAIA::NUM sSize, GAIA::NUM sOffset, GAIA::NUM& sNext) const
		{
			for(GAIA::NUM x = sOffset; x < sSize; ++x)
			{
				if(pBuf[x] == '>')
		...
		GINL GAIA::NUM Json::load_node_name(GAIA::U8* pBuf, GAIA::NUM sSize, GAIA::NUM sOffset, GAIA::NUM& sNext) const
		{
			for(GAIA::NUM x = sOffset; x < sSize; ++x)
			{
		...
		GINL GAIA::NUM Json::load_name(GAIA::U8* pBuf, GAIA::NUM sSize, GAIA::NUM sOffset, GAIA::NUM& sNext) const
		{
			for(GAIA::NUM x = sOffset; x < sSize; ++x)
			{
		...
		GINL GAIA::NUM Json::load_value(GAIA::U8* pBuf, GAIA::NUM sSize, GAIA::NUM sOffset, GAIA::NUM& sNext) const
		{
			for(GAIA::NUM x = sOffset; x < sSize; ++x)
			{
				if(pBuf[x] == '\"')
		...
		GINL GAIA::GVOID Json::read_string(const GAIA::U8* pBegin, const GAIA::U8* pEnd, GAIA::CTN::WString& strResult) const
		{
			GAST(pEnd - pBegin > 0);
