...
#define XDELTA3_MIN_MATCH 4U  /* VCDIFF code table: XDELTA3_MIN_MATCH=4 */
#define MIN_ADD           1U  /* 1 */
...
struct _xd3_code_table_desc
{
  ...
  /* Assumes that XDELTA3_MIN_MATCH is 4 */
...
xd3_build_code_table (const xd3_code_table_desc *desc, xd3_dinst *tbl)
{
  ...
  for (mode = 0; mode < cpy_modes; mode += 1)
    {
      ...
      for (size1 = XDELTA3_MIN_MATCH; size1 < XDELTA3_MIN_MATCH + desc->cpy_sizes; size1 += 1, d += 1)
	{
	  d->type1 = XD3_CPY + mode;
	  d->size1 = size1;
	}
  ...
  for (mode = 0; mode < cpy_modes; mode += 1)
      ...
      for (size1 = 1; size1 <= desc->addcopy_add_max; size1 += 1)
	{
	  ...
	  for (size2 = XDELTA3_MIN_MATCH; size2 <= max; size2 += 1, d += 1)
	    {
	      d->type1 = XD3_ADD;
	      d->size1 = size1;
  ...
  for (mode = 0; mode < cpy_modes; mode += 1)
	...
	desc->copyadd_same_cpy_max;
      ...
      for (size1 = XDELTA3_MIN_MATCH; size1 <= max; size1 += 1)
	{
...
static unsigned long xd3_internal_adler32 (unsigned long adler, const uint8_t *buf,
			      usize_t len)
{
		  ...
		  xd3_config *config)
{
  ...
  switch (config->smatch_cfg)
	...
	*smatcher = config->smatcher_soft;
	...
	if (smatcher->large_look  < XDELTA3_MIN_MATCH ||
	    smatcher->large_step  < 1         ||
	    smatcher->small_look  < XDELTA3_MIN_MATCH)
	  {
		...
		xd3_source *src)
{
  ...
  if (xd3_check_pow2 (src->blksize, &shiftby) != 0)
    {
      src->blksize = (usize_t)xd3_pow2_roundup(src->blksize);
...
xd3_iopt_finish_encoding (xd3_stream *stream, xd3_rinst *inst)
{
  ...
  switch (inst->type)
	...
	/* Note: used to assert inst->size >= XDELTA3_MIN_MATCH, but not true
    ...
    case XD3_RUN:
      {
	XD3_ASSERT (inst->size >= XDELTA3_MIN_MATCH);
...
xd3_iopt_flush_instructions (xd3_stream *stream, int force)
{
  ...
  while (! xd3_rlist_end (& stream->iopt_used, r1) &&
      ...
      else if (! force)
	{
      ...
      if (gap < 2*XDELTA3_MIN_MATCH || r2moff <= 2 || r2off <= 2)
	{
      ...
      else
	{
	  ...
	  XD3_ASSERT (r1->size >= XDELTA3_MIN_MATCH);
	  XD3_ASSERT (r2->size >= XDELTA3_MIN_MATCH);
...
#     define ENCODE_SECONDARY_SECTION(UPPER,LOWER) \
  ...
  if (use_adler32)
	...
	{
	  a32 = (uint32_t)xd3_internal_adler32 (1L, stream->next_in, stream->avail_in);
	}
      else
	{
...
xd3_encode_input (xd3_stream *stream)
{
  ...
  switch (stream->enc_state)
	  ...
	  (stream->avail_in < stream->winsize && ! (stream->flags & XD3_FLUSH)))
	{
      ...
      stream->min_match = XDELTA3_MIN_MATCH;
  ...
  if (src->eof_known)
    {
      ...
      src->srclen = (usize_t)xd3_min (src->srclen, xd3_source_eof(src) - src->srcbase);
    }
 ...
 * (stream->min_match > XDELTA3_MIN_MATCH) then a lazy match is in effect.
 */
static usize_t
	    ...
	    usize_t *match_offset)
{
  ...
  usize_t chain = (stream->min_match == XDELTA3_MIN_MATCH ?
...
XD3_TEMPLATE(xd3_string_match_) (xd3_stream *stream)
{
 ...
 restartloop:
  ...
  if (xd3_iopt_last_matched (stream) > stream->input_position)
    {
      stream->min_match = xd3_max (XDELTA3_MIN_MATCH,
...
#define HANDLELAZY(mlen) \
  ...
  for (;; inp += 1, stream->input_position += 1)
      ...
      if (DO_SMALL)
	{
       ...
       * shortens as well.  */
      if (stream->min_match > XDELTA3_MIN_MATCH)
	{
	  stream->min_match -= 1;
	}
