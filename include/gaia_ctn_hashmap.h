#ifndef		__GAIA_CTN_HASHMAP_H__
#define		__GAIA_CTN_HASHMAP_H__
...
namespace GAIA
{
	namespace CTN
	{
		template<typename _KeyType, typename _DataType, typename _SizeType, typename _HashType, typename _ExtendType> class BasicHashMap : public GAIA::Base
		{
		public:
		...
		public:
			typedef BasicHashMap<_KeyType, _DataType, _SizeType, _HashType, _ExtendType> __MyType;
		private:
		...
		public:
			...
			public:
				...
				const _KeyType* key() const{if(this->empty()) return GNIL; return &m_pNode->k;}
			private:
		...
		public:
			...
			GINL GAIA::BL insert(const _KeyType& k, const _DataType& t)
			{
			...
			GINL const_it const_findit(const _KeyType& k) const
			{
				const_it ret;
				Node* pNode = this->findnode(k);
				if(pNode == GNIL)
		...
		private:
			...
			GINL Node* findnode(const _KeyType& k) const
			{
				if(m_groups.empty())
					return GNIL;
				_HashType h = (_HashType)GAIA::ALGO::hash(k);
		...
		template<typename _KeyType, typename _DataType> class HashMap : public BasicHashMap<_KeyType, _DataType, GAIA::NUM, GAIA::U64, GAIA::ALGO::ExtendGold<GAIA::NUM> >{public:};
	}
}
