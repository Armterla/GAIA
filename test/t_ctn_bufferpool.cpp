...
namespace TEST
{
	...
	{
		typedef GAIA::CTN::BufferPool __BufferPoolType;
		__BufferPoolType bp;
		if(!bp.empty())
			TERROR;
		if(bp.size() != 0)
		...
		if(pBufT != pBuf)
			TERROR;
		if(!bp.release(pBufT, sizeof("HelloWorld")))
			TERROR;
		...
		__BufferPoolType bp1 = bp;
		if(bp1.size() != 2)
			TERROR;
		if(bp1.buffer_size() != 2)
			TERROR;
		if(!bp.release(pBuf, sizeof("HelloWorld")))
			TERROR;
		if(!bp.release(pBuf1, sizeof("HelloWorld")))
			TERROR;
		if(!bp.release(pBuf2, sizeof("HelloKitty")))
			TERROR;
		if(!bp.release(pBuf3, sizeof("HelloKitty")))
			TERROR;
		for(GAIA::NUM x = 0; x < bp.buffer_size(); ++x)
		{
			GAIA::NUM sSize;
