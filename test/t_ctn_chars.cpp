...
namespace TEST
{
	extern GAIA::GVOID t_ctn_chars(GAIA::LOG::Log& logobj)
	{
		...
		if(GAIA::ALGO::gstrcmp(szTemp, "lo") != 0)
			TERROR;
		chs1.clear();
		if(chs.extract_left(5, chs1) != 5)
			TERROR;
		if(chs1 != "Hello")
			TERROR;
		chs1.clear();
		if(chs.extract_right(5, chs1) != 6)
			TERROR;
		if(chs1 != "World!")
			TERROR;
		chs1.clear();
		if(chs.extract_mid(4, 6, chs1) != 3)
			TERROR;
		if(chs1 != "o W")
			TERROR;
		chs1 = chs.extract_left(5);
		if(chs1 != "Hello")
			TERROR;
		chs1 = chs.extract_right(5);
		if(chs1 != "World!")
			TERROR;
		chs1 = chs.extract_mid(4, 6);
		if(chs1 != "o W")
			TERROR;
		if(chs.extract_left(5, szTemp, sizeofarray(szTemp)) != 5)
			TERROR;
		if(GAIA::ALGO::gstrcmp(szTemp, "Hello") != 0)
			TERROR;
		if(chs.extract_right(5, szTemp, sizeofarray(szTemp)) != 6)
			TERROR;
		if(GAIA::ALGO::gstrcmp(szTemp, "World!") != 0)
			TERROR;
		if(chs.extract_mid(4, 6, szTemp, sizeofarray(szTemp)) != 3)
			TERROR;
		...
		chs.assign("Hello World");
		if(chs != "Hello World")
			TERROR;
		chs.append("Hello Kitty");
		if(chs != "Hello WorldHello Kitty")
			TERROR;
		chs.assign("Hello World", 5);
		if(chs != "Hello")
			TERROR;
		chs.append("Hello Kitty", 5);
		if(chs != "HelloHello")
			TERROR;
