...
HPDF_Stream_Tell  (HPDF_Stream  stream)
{
    ...
    if (!(stream->tell_fn))
        return (HPDF_INT32)HPDF_SetError (stream->error, HPDF_INVALID_OPERATION, 0);
...
HPDF_FileStream_TellFunc  (HPDF_Stream   stream)
{
    ...
    if ((ret = (HPDF_INT32)HPDF_FTELL (fp)) < 0) {
        return (HPDF_INT32)HPDF_SetError (stream->error, HPDF_FILE_IO_ERROR,
                HPDF_FERROR(fp));
    }
...
HPDF_FileStream_SizeFunc  (HPDF_Stream   stream)
{
    ...
    if ((ptr = (HPDF_INT)HPDF_FTELL (fp)) < 0) {
    ...
    if ((size = (HPDF_INT)HPDF_FTELL (fp)) < 0) {
        HPDF_SetError (stream->error, HPDF_FILE_IO_ERROR,
                HPDF_FERROR(fp));
        return 0;
    }
